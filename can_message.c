...
int check_start(const CanMessage* message){
    if((message->id != 0x0A0) || (message->payload_bytes!= 2)){
        return 0;
    }   
    if(message->payload[0] == 0x66 && message->payload[1] == 0x01){
        return 1;
    }
    if(message->payload[0] == 0xFF && message->payload[1] == 0x01){
        return 1;
    }
    return 0;
}
